...
# NOTE:
# KIS overseas sector backfill tends to write Korean sector names which will
# mix languages in the dashboard. Default is OFF; enable explicitly if needed.
if [ "${SECTOR_CLASSIFIER_USE_KIS:-0}" = "1" ]; then
  $PYBIN -m src.collectors.us_sector_kis_backfill --build-csv > logs/sector_kis_backfill.log 2>&1
else
  echo "[sector] skip us_sector_kis_backfill (set SECTOR_CLASSIFIER_USE_KIS=1 to enable)" > logs/sector_kis_backfill.log
fi
