...
ICB_TO_GICS_SECTOR = {
    # Wikipedia Nasdaq-100 table currently provides ICB Industry for constituents.
    # Map it into the GICS-style names used across the viewer DB.
...
def fetch_sp500() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    secmap = pd.DataFrame({
        "code": t[sym].astype(str).str.strip(),
        "sector_name": t[sector].apply(_clean_text) if sector else None,
...
def fetch_nasdaq100() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    icb_sector = next((c for k, c in cols.items() if "icb industry" in k), None)
    icb_industry = next((c for k, c in cols.items() if "icb subsector" in k), None)
    ...
    raw_industry = None
    if sector:
        raw_sector = t[sector].apply(_clean_text)
        raw_industry = t[industry].apply(_clean_text) if industry else None
    elif icb_sector:
        raw_sector = t[icb_sector].apply(_clean_text)
        raw_industry = t[icb_industry].apply(_clean_text) if icb_industry else None

    mapped_sector = None
    if raw_sector is not None:
        mapped_sector = raw_sector.apply(lambda v: ICB_TO_GICS_SECTOR.get(str(v).strip(), v) if v else None)

    secmap = pd.DataFrame({
        "code": t[ticker].astype(str).str.strip(),
        "sector_name": mapped_sector if mapped_sector is not None else None,
        "industry_name": raw_industry if raw_industry is not None else None,
...
def main():
    ...
    # Prefer SP500(GICS) sector labels for codes that appear in both lists.
    secmap = pd.concat([sec_sp, sec_nas], ignore_index=True)
