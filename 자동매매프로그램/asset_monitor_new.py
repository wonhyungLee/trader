    ...
    def format_asset_message(self, crypto_assets: Dict, stock_assets: Dict) -> Dict:
        ...
        if stock_assets:
            ...
            for account, data in stock_assets.items():
                ...
                acc_total = data.get("total_krw")
                if acc_total is None:
                    acc_total = dom.get("total_krw", 0)
                ...
                total_dom_value += acc_total or 0
                total_dom_pnl += acc_pnl or 0
                total_ovrs_value += ovrs.get("total_krw", 0) or 0
                total_futures_pnl_krw += fut.get("total_pnl_krw", 0) or 0

                lines = []
                lines.append(f"국내 평가 {acc_total:,.0f}원 (손익 {acc_pnl:,.0f}원, {acc_rate:+.2f}%){cash_text}")
            ...
            summary_lines = [
                f"총 국내 평가액 {total_dom_value:,.0f}원 / 총 국내 손익 {total_dom_pnl:,.0f}원",
                f"총 해외주식 환산 {total_ovrs_value:,.0f}원",
                f"총 해외선물 손익(원화) {total_futures_pnl_krw:,.0f}원",
            ]
            embed["fields"].append({
                "name": "요약",
                "value": "\n".join(summary_lines),
