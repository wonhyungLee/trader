...
from utils.fx_provider import get_rates
...
class KISClient:
    ...
    def get_fx_rates(self) -> Dict[str, float]:
        """USD/HKD/JPY 환율 조회 (당일 일봉, 시장분류 X) + 캐시."""

        def fetch_tr() -> Dict[str, float]:
            today = datetime.now().strftime("%Y%m%d")
            currencies = ["USD", "HKD", "JPY"]
            rates: Dict[str, float] = {}

            logger.info(f"KIS{self.kis_number}: 환율 조회 시작 (TR, {', '.join(currencies)})")
            for cur in currencies:
                ...
                try:
                    ...
                    if output1:
                        ...
                        if rate > 0:
                            rates[cur] = rate
                            logger.info(f"KIS{self.kis_number}: 환율 {cur}={rate:,.4f}")
                        else:
                            logger.warning(f"KIS{self.kis_number}: 환율 {cur} 값이 0/미인식 output={output1[-1]}")
                    else:
                        logger.warning(f"KIS{self.kis_number}: 환율 {cur} output 없음")
                except Exception as e:
                    logger.error(f"KIS{self.kis_number} 환율 조회 실패({cur}): {e}")
            return rates

        # MCP 호출자는 아직 없으므로 TR→캐시 순으로 시도
        rates, meta = get_rates(fetch_tr_fn=fetch_tr, fetch_mcp_fn=None)
        self.last_fx_meta = meta
        if not rates:
            logger.warning(f"KIS{self.kis_number}: 환율 조회 결과가 비어 있습니다.(meta={meta})")
        return rates
    ...
    def get_overseas_balance(self) -> Dict:
        ...
        stocks: List[Dict] = []
        seen_symbols: set[Tuple[str, str]] = set()  # (currency, symbol)
        duplicate_logged: set[Tuple[str, str]] = set()
            ...
            for _ in range(6):
                ...
                for item in output1:
                    ...
                    if key in seen_symbols:
                        if key not in duplicate_logged:
                            logger.info(
                                f"KIS{self.kis_number}: 중복 해외주식 응답 스킵 ({symbol}/{currency}, exchange {exchange})"
                            )
                            duplicate_logged.add(key)
                        continue
        ...
        return {
            ...
            "stocks": stocks,
            "fx_rates": fx_rates,
            "fx_meta": getattr(self, "last_fx_meta", {}),
            "total_krw": krw_total,
        }
    ...
    def get_balance(self) -> Dict:
        try:
            ...
            # 해외선물 조회를 중지했으므로 해외 주식 환율만 사용한다.
            fx_rates: Dict[str, float] = overseas.get("fx_rates") or {}

            # 해외 주식 총평가액을 환율로 다시 계산해 반영
            ovrs_total_krw = 0.0
            for cur, vals in (overseas.get("per_currency") or {}).items():
                rate = fx_rates.get(cur)
                if rate and vals.get("total_eval"):
                    ovrs_total_krw += vals.get("total_eval", 0.0) * rate
            overseas["total_krw"] = ovrs_total_krw
            overseas["fx_rates"] = fx_rates
            # 메타 데이터는 해외선물(중지됨) 대신 해외주식 소스만 사용
            overseas["fx_meta"] = overseas.get("fx_meta") or {}

            return {
                "domestic": domestic,
                "overseas": overseas,
                "futures": futures,
                "total_krw": domestic.get("total_krw", 0) + ovrs_total_krw,
