...
def _load_sector_seed(store: SQLiteStore, path: str) -> int:
    ...
    for _, r in df.iterrows():
        ...
        if not code:
            continue
        sector_name = str(r.get(sector_col, "")).strip() if sector_col else ""
        industry_name = str(r.get(industry_col, "")).strip() if industry_col else ""
        # Important: avoid wiping existing DB sector_map with empty seed rows.
