...
- 기본: data/universe_kospi100.csv + data/universe_kosdaq150.csv
...
def _auto_rank_universe(store: SQLiteStore, top_kospi: int, top_kosdaq: int) -> pd.DataFrame:
    ...
    kospi = kospi.assign(group_name="KOSPI100")
...
def main(args: argparse.Namespace):
    ...
    try:
        if args.auto_rank:
            ...
            df[df["group_name"] == "KOSPI100"].to_csv("data/universe_kospi100.csv", index=False)
        ...
        if len(df) != 250:
            raise RuntimeError(f"universe count must be 250, got {len(df)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--top-kospi", type=int, default=100, help="auto_rank KOSPI 상위 N")
