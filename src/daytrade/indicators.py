...
"""Indicator helpers for the day-trade planner.

We intentionally keep implementations simple and dependency-free (numpy only)
to make automation deployment easier.

Definitions match the earlier backtest assumptions:
  - RSI: SMA of gains/losses (Connors RSI(2) style)
...
def rsi_sma(values: Sequence[float], period: int = 2) -> np.ndarray:
    """RSI using simple average of the last `period` gains/losses.
