...
이 문서는 유니버스(KOSPI 100 + KOSDAQ 150) 250종목에 대해 데이터 리필 및 증분 업데이트를 수행하는 절차를 설명합니다.
...
```bash
# 또는 ./run_refill.sh 실행
python -m src.collectors.refill_loader --start-mode listing --resume
```
* **동작 원리**: KIS **주식기본조회**에서 상장일을 가져와 그 날짜를 하한으로 설정하고, 오늘부터 과거로 chunk를 나누어 호출합니다. 상장일이 확인되지 않으면 기존처럼 역방향 스캔으로 진행합니다.
